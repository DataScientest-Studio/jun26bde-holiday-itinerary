...
# ==========================================
# 📂 VIEW SAVED TRIPS (ALWAYS VISIBLE AT TOP)
# ==========================================
with st.expander("📂 View My Saved Trips", expanded=False):
    ...
    with col_btn_load:
        st.write("")
        st.write("")
        if st.button("Load Trips", use_container_width=True):
            if not load_email:
                        ...
                        for idx, trip in enumerate(saved_trips):
                            with st.container(border=True):
                                ...
                                for day, stops in days.items():
                                    st.write(f"**Day {day}**")
                                    for stop in stops:
                                        city_str = stop.get('city') or 'Unknown location'
                                        st.caption(f"{stop['stop_order']}. {stop['label']} — 📍 {city_str}")
                else:
...
if st.button("Generate itinerary", type="primary"):
    ...
    if response.status_code != 200:
        ...
        # Save to session state for the Save Trip feature
...
# ==========================================
# 💾 SAVE CURRENT TRIP (APPEARS AFTER GENERATING)
# ==========================================
